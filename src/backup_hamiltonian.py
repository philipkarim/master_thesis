...
def pairing_hamiltonian(n_states,d_energy,g):
    ...
    for i in range(n_states):
        ...
        if i<int(n_states/2) and g!=0:
            ...
            V_gates.append([level_energy,[2*i,'z']])
            V_gates.append([level_energy, [2*i+1,'z']])
            V_gates.append([level_energy,[2*i,'z'],[2*i+1,'z']])

            

        #Non diagonal two body interaction
        for j in range(i+1,int(n_states/2)):
            V_gates.append([-(1/16)*g,[2*i,'x'],[2*i+1,'x'],[2*j,'x'],[2*j+1,'x']])
            V_gates.append([(1/16)*g,[2*i,'x'],[2*i+1,'x'],[2*j,'y'],[2*j+1,'y']])
            V_gates.append([-(1/16)*g,[2*i,'x'],[2*i+1,'y'],[2*j,'x'],[2*j+1,'y']])
            V_gates.append([-(1/16)*g,[2*i,'x'],[2*i+1,'y'],[2*j,'y'],[2*j+1,'x']])
            V_gates.append([-(1/16)*g,[2*i,'y'],[2*i+1,'x'],[2*j,'x'],[2*j+1,'y']])
            V_gates.append([-(1/16)*g,[2*i,'y'],[2*i+1,'x'],[2*j,'y'],[2*j+1,'x']])
            V_gates.append([(1/16)*g,[2*i,'y'],[2*i+1,'y'],[2*j,'x'],[2*j+1,'x']])
            V_gates.append([-(1/16)*g,[2*i,'y'],[2*i+1,'y'],[2*j,'y'],[2*j+1,'y']])

    H_gates.extend(V_gates)
    H_gates.append([total_energy])
    #print(hamiltonian_list==H_gates)
...
circ_list=pairing_hamiltonian(4,3,2)
circ_list_sol=pairing_hamiltonian_solution(4,3,2)
print(circ_list)
print(circ_list_sol)
